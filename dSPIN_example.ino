...
#define dSPIN_BUSYN      7   // Wire this to the BSYN line

//#define STAT1              // Hooked to an LED on the test jig
...
void setup() 
{
  ...
  if (dSPIN_GetParam(dSPIN_CONFIG) == 0x2E88) 
    {
      ...
      Serial.println(dSPIN_GetParam(dSPIN_CONFIG));
    }
    else
    {
  ...
  //  value will be used.
  dSPIN_SetParam(dSPIN_MAX_SPEED, MaxSpdCalc(850));
  ...
  dSPIN_SetParam(dSPIN_ACC, AccCalc(200));
  
  dSPIN_SetParam(dSPIN_DEC, DecCalc(400));
  ...
  //  Shut down bridges on overcurrent
  ...
  dSPIN_SetParam(dSPIN_KVAL_ACC, 0x7F);
  dSPIN_SetParam(dSPIN_KVAL_DEC, 0x7F);
  dSPIN_SetParam(dSPIN_KVAL_HOLD, 0x05);
